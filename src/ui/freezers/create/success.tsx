...
export default function AddFreezerSuccess({id}: {id: number}) {
  ...
  useEffect(() => {
    setTimeout(() => router.replace("/freezers/3", { scroll: false }), REDIRECT_DELAY);
  })

  return (
    <div className="flex flex-col text-center justify-center items-center">
      <h1 className="text-green-500 text-5xl animate-pulse"><FaCircleCheck /></h1>
      <h2 className="text-2xl mt-4">Created freezer {"'"}{freezers[0].name}{"'"}</h2>
      ...
      <p className="text-xs mt-1 text-gray-300">Click <Link className="underline" href={"/freezers/3"} scroll={false}>here</Link> it you are stuck</p>
    </div>
  )
}
